...
function deploymentCfg(){
  const c=window.PH_CONFIG||{};
...
function getCfg(){
  const deployed=deploymentCfg();
  if(deployed.url&&deployed.key)return deployed;
...
function loadSettings(){
  const c=getCfg(),deployed=deploymentCfg();$("#supabaseUrl").value=c.url||"";$("#supabaseKey").value=c.key||"";
  if(deployed.url&&deployed.key){
    $("#supabaseUrl").readOnly=true;$("#supabaseKey").readOnly=true;
    $("#saveSupabase").classList.add("hidden");
    $("#supabaseStatus").textContent="เชื่อมต่ออัตโนมัติจากการตั้งค่าของแอป";
  }
...
$("#bodyLogForm").onsubmit=e=>{e.preventDefault();const s=$("#bodyLogStatus"),val=id=>numOrText($(id).value),log={id:uid(),date:$("#wmDate").value,weight:val("#wmWeight"),waist:val("#wmWaistInput"),calories:val("#wmCaloriesEaten"),protein:val("#wmProteinEaten"),steps:val("#wmStepsDone"),cardio:val("#wmCardio"),strength:val("#wmStrength"),created_at:new Date().toISOString()};if(!log.date){s.textContent="กรุณาเลือกวันที่";return}if(![log.weight,log.waist,log.calories,log.protein,log.steps,log.cardio,log.strength].some(x=>x!==null)){s.textContent="กรอกอย่างน้อย 1 ค่า";return}if(log.weight!==null&&(log.weight<20||log.weight>400)){s.textContent="น้ำหนักต้องอยู่ระหว่าง 20–400 kg";return}db.bodyLogs.push(log);if(log.weight!==null)db.records.push({id:uid(),date:new Date(log.date+"T08:00:00").toISOString(),type:"weight",value:log.weight,value2:null,unit:"kg",note:"Weight Management v9.7",created_at:new Date().toISOString(),updated_at:new Date().toISOString()});save();e.target.reset();$("#wmDate").value=new Date().toISOString().slice(0,10);s.textContent="บันทึกข้อมูลวันนี้สำเร็จ"};
...
if($("#bpSessionForm"))$("#bpSessionForm").onsubmit=e=>{e.preventDefault();const s=$("#bpFormStatus"),x={id:uid(),date:$("#bpSessionDate").value,period:$("#bpPeriod").value,sys1:Number($("#bpSys1").value),dia1:Number($("#bpDia1").value),sys2:Number($("#bpSys2").value),dia2:Number($("#bpDia2").value),rested:$("#bpRested").checked,created_at:new Date().toISOString()};if(!x.date){s.textContent="กรุณาเลือกวันที่";return}if([x.sys1,x.sys2].some(v=>v<50||v>300)||[x.dia1,x.dia2].some(v=>v<30||v>200)){s.textContent="ค่าความดันอยู่นอกช่วง กรุณาตรวจตัวเลข";return}if(x.sys1<=x.dia1||x.sys2<=x.dia2){s.textContent="SYS (ตัวบน) ต้องมากกว่า DIA (ตัวล่าง) ทั้ง 2 ครั้ง";return}const m=bpSessionAvg(x);db.bpSessions.push(x);db.records.push({id:uid(),date:new Date(x.date+(x.period==="morning"?"T08:00:00":"T20:00:00")).toISOString(),type:"blood_pressure",value:Number(m.sys.toFixed(1)),value2:Number(m.dia.toFixed(1)),unit:"mmHg",note:`ค่าเฉลี่ยจากการวัด 2 ครั้ง (${x.period==="morning"?"เช้า":"เย็น"}) v9.7`,created_at:new Date().toISOString(),updated_at:new Date().toISOString()});save();e.target.reset();$("#bpSessionDate").value=new Date().toISOString().slice(0,10);s.textContent=`บันทึกสำเร็จ • ค่าเฉลี่ย ${Math.round(m.sys)}/${Math.round(m.dia)} mmHg`};
