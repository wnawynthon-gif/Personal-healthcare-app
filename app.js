...
function renderDoctor(){
 ...
 <p class="muted">Generated by Personal Healthcare v6.1 from user-entered data. This summary is not a diagnosis and does not replace the original laboratory report or clinician medication list.</p>`;
}
...
if(false && id('labCsvFile')) id('labCsvFile').onchange=async e=>{
 const f=e.target.files?.[0]; if(!f)return;
 try{
...
// ---------- v6.1 Safe Lab CSV Import: Preview -> Confirm ----------
let labImportPreviewRows=[];
function csvRows(text){
 const rows=[];let row=[],cell='',q=false;
 for(let i=0;i<text.length;i++){const ch=text[i];if(q){if(ch==='"'&&text[i+1]==='"'){cell+='"';i++}else if(ch==='"')q=false;else cell+=ch}else{if(ch==='"')q=true;else if(ch===','){row.push(cell.trim());cell=''}else if(ch==='\n'||ch==='\r'){if(ch==='\r'&&text[i+1]==='\n')i++;row.push(cell.trim());cell='';if(row.some(x=>x!==''))rows.push(row);row=[]}else cell+=ch}}
 row.push(cell.trim());if(row.some(x=>x!==''))rows.push(row);return rows;
}
function normalizeImportDate(v){v=String(v||'').trim();if(/^\d{4}-\d{2}-\d{2}$/.test(v))return v;const m=v.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{4})$/);if(m)return`${m[3]}-${m[2].padStart(2,'0')}-${m[1].padStart(2,'0')}`;return v}
function parseLabCsvV61(text){const rows=csvRows(text);if(rows.length<2)throw new Error('CSV ไม่มีข้อมูล');const h=rows[0].map(x=>x.trim().toLowerCase().replace(/\s+/g,'_'));const ix=n=>h.indexOf(n);for(const n of ['name','value','date'])if(ix(n)<0)throw new Error('ต้องมีคอลัมน์ name, value, date');return rows.slice(1).map((c,i)=>{const get=n=>ix(n)>=0?(c[ix(n)]||'').trim():'';const x={name:get('name'),value:get('value'),unit:get('unit'),date:normalizeImportDate(get('date')),refLow:get('ref_low'),refHigh:get('ref_high'),refMode:get('ref_mode')||'none',range:get('range'),category:get('category')||'auto',row:i+2};x.valid=!!(x.name&&x.value&&/^\d{4}-\d{2}-\d{2}$/.test(x.date));return x}).filter(x=>x.name||x.value||x.date)}
function renderLabImportPreview(){const el=id('labImportPreview'),btn=id('confirmLabImportBtn'),clr=id('clearLabPreviewBtn'),st=id('labImportStatus');if(!el)return;const good=labImportPreviewRows.filter(x=>x.valid).length,bad=labImportPreviewRows.length-good;st.innerHTML=`พบ <strong>${labImportPreviewRows.length}</strong> รายการ • พร้อมนำเข้า <strong>${good}</strong>${bad?` • ต้องแก้ CSV <strong>${bad}</strong>`:''}`;el.innerHTML=labImportPreviewRows.length?`<div class="import-table-wrap"><table class="doctor-table import-table"><tr><th>สถานะ</th><th>วันที่</th><th>รายการ</th><th>ค่า</th><th>Ref.</th></tr>${labImportPreviewRows.map(x=>`<tr class="${x.valid?'':'import-invalid'}"><td>${x.valid?'✓':'!'}</td><td>${esc(x.date)}</td><td>${esc(x.name)}<small>${esc(x.category)}</small></td><td>${esc(x.value)} ${esc(x.unit)}</td><td>${esc(x.refLow||'–')} – ${esc(x.refHigh||'–')} ${esc(x.range)}</td></tr>`).join('')}</table></div>`:'';btn.style.display=good?'block':'none';btn.textContent=`ยืนยันนำเข้า ${good} รายการ`;clr.disabled=!labImportPreviewRows.length}
if(id('labCsvFileV61'))id('labCsvFileV61').onchange=async e=>{const f=e.target.files?.[0];if(!f)return;try{labImportPreviewRows=parseLabCsvV61(await f.text());renderLabImportPreview()}catch(err){labImportPreviewRows=[];renderLabImportPreview();alert('อ่าน CSV ไม่สำเร็จ: '+err.message)}};
if(id('clearLabPreviewBtn'))id('clearLabPreviewBtn').onclick=()=>{labImportPreviewRows=[];if(id('labCsvFileV61'))id('labCsvFileV61').value='';renderLabImportPreview()};
if(id('confirmLabImportBtn'))id('confirmLabImportBtn').onclick=()=>{const good=labImportPreviewRows.filter(x=>x.valid);if(!good.length)return;const base=Date.now();good.forEach((x,i)=>db.labs.push({id:base+i,name:x.name,value:x.value,unit:x.unit,date:x.date,range:x.range,refLow:x.refLow===''?null:Number(x.refLow),refHigh:x.refHigh===''?null:Number(x.refHigh),refMode:x.refMode,category:x.category}));save();labImportPreviewRows=[];renderLabImportPreview();renderAll();alert(`นำเข้าและวิเคราะห์สำเร็จ ${good.length} รายการ`)};
