...
// ---------- v6.9 Build / Deployment Guard ----------
const PH_BUILD_VERSION='6.9';
const PH_BUILD_ID='6.9.0-20260810';
const PH_OCR_ENGINE='V6.8 Calibrated Multi-pass';
...
function renderDoctor(){
 ...
 <p class="muted">Generated by Personal Healthcare v6.9 from user-entered data. This summary is not a diagnosis and does not replace the original laboratory report or clinician medication list.</p>`;
}
id('printBtn').onclick=()=>window.print();
id('exportBtn').onclick=()=>{
 const blob=new Blob([JSON.stringify(db,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='personal-health-data-v6.9.json';a.click();URL.revokeObjectURL(a.href)
};
...
},'personal-healthcare-v6.9-backup.json');
...
async function v622ProcessFiles(files){
 ...
 try{
   ...
   setOcrProgress('อ่านเอกสารเสร็จแล้ว — V6.9 • OCR Engine V6.8 Multi-pass: อ่าน Result 3 รอบและใช้เฉพาะค่าที่ตรงกัน 2/3 รอบ',100);v622RenderDocumentMeta();renderDetectedRows();
