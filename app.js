...
function renderDoctor(){
 ...
 <p class="muted">Generated by Personal Healthcare v6.4 from user-entered data. This summary is not a diagnosis and does not replace the original laboratory report or clinician medication list.</p>`;
}
...
if(!localStorage.getItem(V64_MIGRATION_KEY)){
 db.labs=(db.labs||[]).map(l=>{
   const legacy=String(l?.source||'').startsWith('document-import-v6.') && l?.source!=='document-import-v6.4';
...
function detectLabRowsFromText(text){
 ...
 for(const spec of V63_LABS){
   ...
   rows.push({selected:false,name:spec.name,value:hit.value,unit:spec.unit,date,range,refLow:spec.low,refHigh:spec.high,refMode:mode,category:spec.cat,confidence:hit.quality,reviewRequired:hit.quality!=='high'});
 }
 return rows;
}
function v63PlausibleRow(r){
 const spec=V63_LABS.find(x=>x.name===r.name); if(!spec)return false;
 const v=+r.value; if(!Number.isFinite(v)||v<spec.min||v>spec.max)return false;
 // Common OCR failure: reference limit is captured as the patient result. Force manual review instead of auto-saving.
 if(spec.high!=null&&Math.abs(v-spec.high)<1e-9)return false;
 if(spec.low!=null&&spec.high!=null&&Math.abs(v-spec.low)<1e-9)return false;
 return true;
}
...
async function v622ProcessFiles(files){
 ...
 try{
   ...
   setOcrProgress('อ่านเอกสารเสร็จแล้ว — V6.4 ไม่เลือกผล OCR อัตโนมัติ กรุณาติ๊กเฉพาะค่าที่ตรงกับใบจริง',100);v622RenderDocumentMeta();renderDetectedRows();
...
function v64QuarantineLegacyOcr(){
 let n=0;
 db.labs=(db.labs||[]).map(l=>{
  if(String(l?.source||'').startsWith('document-import-v6.') && l.source!=='document-import-v6.4' && l.importStatus!=='needs-review'){
   n++; return {...l,importStatus:'needs-review',qualityReason:'Legacy OCR import quarantined manually in v6.4'};
  }
  return l;
 });
